...
- indicator presence/absence of the layers of less then 2 m
...
#### Serenity
- if distant > 400m from road at 70km/h
- variable derived from spatial data


#### Diversity
- Volume for individual species & count; Volume share by species? If > 70%, then not a diverse forest

#### Nature
- mean age > 100 years; Age_VOL, Age_BA (lower than Age_vol)
...
- **BAU_GTR** - business as usual, with retained trees after final farvest
...
### SIMO indicators classification



### Diversity: 
- mixed if less then 70% of the volume belong to one species

#### Tree species: (by volume)
...
```{r diversity, echo=FALSE, fig.dim = c(7, 3), include=FALSE}
...
```{r plot-diversity, echo = F, include=FALSE, fig.dim = c(7, 3)}
...
```{r plot-richness, echo = F, include=FALSE, fig.dim = c(7, 3)}
...
```{r hist-age, fig.dim = c(7, 3), include=FALSE, message=FALSE}
...
```{r plot-natural, echo = F, fig.dim = c(7, 3), message=FALSE}
...
```{r cohesion, echo=FALSE, fig.dim = c(7, 3), include=FALSE}
